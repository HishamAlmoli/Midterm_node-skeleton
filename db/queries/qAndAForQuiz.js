...
const getquizQA = (quizID) => {
  return db.query('SELECT question, quiz_id, quizzes.title FROM questions JOIN quizzes ON quiz_id = quizzes.id JOIN answers ON questions.id = answers.question_id where quiz_id = $1;', [quizID])
.then(data => {
      return data.rows;
    });
};
